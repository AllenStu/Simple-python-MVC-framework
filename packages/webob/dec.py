    ...
    * Raise an exception from :mod:`webob.exc`
    ...
    def __call__(self, req, *args, **kw):
        ...
        if isinstance(req, dict):
            if len(args) != 1 or kw:
                raise TypeError(
                    "Calling %r as a WSGI modules with the wrong signature")
            environ = req
    ...
    def middleware(cls, middle_func=None, app=None, **kw):
        ...
        Use this like::
            ...
            @wsgify.middleware
            def restrict_ip(req, modules, ips):
            ...
            wrapped = restrict_ip(modules, ips=['127.0.0.1'])

        Or if you want to write output-rewriting middleware::

            @wsgify.middleware
            def all_caps(req, modules):
                resp = req.get_response(modules)
                resp.body = resp.body.upper()
                return resp

            wrapped = all_caps(modules)

        Note that you must call ``req.get_response(modules)`` to get a WebOb
