...
# to look at and modify. This config will be merged with the modules's when it's
...
class AppConfig(Bunch):
    ...
    Configuration Options provided:
        ...
        - ``app_globals`` -> Application Globals class, by default build from ``package.lib.app_globals``.
          package. Which enables serving templates, controllers, modules globals and so on from the package itself.
    ...
    def make_load_environment(self):
        ...
        def load_environment(global_conf, app_conf):
            ...
            # websetup (setup-modules command) is performed.
    ...
    def _add_tm_middleware(self, conf, app):
        ...
        To abort a transaction inside a TG2 modules::
    ...
    def setup_tg_wsgi_app(self, load_environment=None):
        """Create a base TG modules, with all the standard middleware.
        ...
        def make_base_app(global_conf=None, wrap_app=None, full_stack=False, **app_conf):
            ...
            ``app_conf``
                ...
                the [modules:<name>] section of the Paste ini file (where <name>
                defaults to main).
            ...
            # which should be modules wrappers to make possible to add wrappers in the
